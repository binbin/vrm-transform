...
type ConvertToKtx2ImageDependenciesInterface interface {
	...
	ParamsGenerator(mode string, width, height int, inputPath, outputPath string, isSRGB bool, etc1sQuality, etc1sClevel, uastcQuality, zstdLevel int) []string
...
type ConvertToKtx2TextureDependenciesInterface interface {
	ConvertToKtx2Image(deps ConvertToKtx2ImageDependenciesInterface, ktx2Mode string, buf []byte, isSRGB bool, etc1sQuality, etc1sClevel, uastcQuality, zstdLevel int) ([]byte, error)
}
